...
{
    private readonly IGamesCatalogService _gamesCatalog;

    public GamesCatalogController(IGamesCatalogService gamesCatalog) =>
        _gamesCatalog = gamesCatalog;
    ...
    public async Task<IActionResult> All(CancellationToken token = default) =>
        await ResponseFromAsync(GameResponse.From(_gamesCatalog.All(token)));
    ...
    public async Task<IActionResult> Get(int gameId, CancellationToken token = default) =>
        await ResponseFromAsync(GameResponse.From(_gamesCatalog.Get(gameId, token)));
}